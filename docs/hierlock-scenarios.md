...
`hier_lock_buckets(level, bucket PRIMARY KEY)` というテーブルの行を、以下の SQL でロックします。
...
- `Resource` は通常は別バケット（別行）なので両立
- よって **待たずに両方取得できる**

注意:
- ストライプ（バケット）方式のため、稀に同じバケットに割り当たると偽競合でブロックする可能性があります
...
- 通常はロックする行（lock_key）が別なので **両立**
- ただしストライプ（バケット）方式のため、稀に同じバケットに割り当たると **偽競合** でブロックする可能性がある
